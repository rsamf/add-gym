...
class PPOAgent(base_agent.BaseAgent):
    ...
    def _decide_action(self, obs, info):
        ...
        norm_action_dist = self.model.eval_actor(norm_obs)
    ...
    def _build_train_data(self):
        ...
        next_vals = torch_util.eval_minibatch(
            self.model.eval_critic, next_critic_inputs, self._critic_eval_batch_size
        )
        next_vals = next_vals.squeeze(-1).detach()

        succ_val = self._compute_succ_val()
        succ_mask = done == base_agent.DoneFlags.SUCC.value
        next_vals[succ_mask] = succ_val

        fail_val = self._compute_fail_val()
        fail_mask = done == base_agent.DoneFlags.FAIL.value
        next_vals[fail_mask] = fail_val

        new_vals = base_agent.compute_td_lambda_return(
        ...
        vals = torch_util.eval_minibatch(
            self.model.eval_critic, critic_inputs, self._critic_eval_batch_size
        )
        ...
        adv_std, adv_mean = torch.std_mean(rand_action_adv, dim=0)
    ...
    def _compute_critic_loss(self, batch):
        ...
        pred = self.model.eval_critic(norm_obs)
        pred = pred.squeeze(-1)
    ...
    def _log_train_info(self, train_info, test_info, env_diag_info, start_time):
        ...
        # All ranks log for distributed aggregation

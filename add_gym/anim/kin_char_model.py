...
class KinCharModel:
    ...
    def load_char_file(self, char_file):
        ...
        # Breadth-first traversal to match Genesis ordering
        # Queue contains tuples of (xml_node, parent_index, is_root)
        queue = [(xml_body_root, -1, True)]

        while queue:
            xml_node, parent_index, is_root = queue.pop(0)
            ...
            for child_body in xml_node.findall("body"):
                queue.append((child_body, curr_index, False))
    ...
    def dof_to_rot(self, dof: torch.Tensor) -> torch.Tensor:
        joint_types = [joint.joint_type for joint in self._joints]
        joint_dof_idx = [joint.dof_idx for joint in self._joints]
        joint_axis = torch.stack(
            [
                (
                    joint.axis
                    if joint.axis is not None
                    else torch.zeros(3, device=self._device)
                )
                for joint in self._joints
            ]
        )
        return kin_char_model_dof_to_rot(joint_types, joint_dof_idx, joint_axis, dof)
...
def kin_char_model_dof_to_rot(
    ...
    for j in range(1, num_joints):
        joint_type = joint_types[j]

        if joint_type == JointType.ROOT:
            dof_dim = 0
        elif joint_type == JointType.HINGE:
            dof_dim = 1
        elif joint_type == JointType.SPHERICAL:
            dof_dim = 3
        elif joint_type == JointType.FIXED:
            ...
            assert False, "Unsupported joint type: {:s}".format(joint_type)

        dof_idx = joint_dof_idx[j]
        j_dof = dof[..., dof_idx : dof_idx + dof_dim]
        axis = joint_axis[j]
        ...
        if joint_type == JointType.ROOT:
            ...
            axis_shape = dof.shape[:-1] + (3,)
            axis = torch.broadcast_to(axis, axis_shape)
            j_dof = j_dof.squeeze(-1)
            joint_rot[..., j - 1, :] = torch_util.axis_angle_to_quat(axis, j_dof)
        elif joint_type == JointType.SPHERICAL:
            joint_rot[..., j - 1, :] = torch_util.exp_map_to_quat(j_dof)
        elif joint_type == JointType.FIXED:
            joint_rot[..., j - 1, :] = 1
        else:
            assert False, "Unsupported joint type: {:s}".format(joint_type)

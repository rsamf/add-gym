...
import torch
import yaml
import add_gym.anim.motion as motion
...
def extract_pose_data(frame):
    root_pos = frame[..., 0:3]
    # xyzw (3,4,5,6) -> wxyz (6,3,4,5)
    root_rot = frame[..., [6, 3, 4, 5]]
    joint_dof = frame[..., 7:]
    return root_pos, root_rot, joint_dof


class MotionLib:
    def __init__(self, motion_file, motion_order, kin_char_model, dt, device):
        self._device = device
        self._kin_char_model = kin_char_model
        self._dt = dt
        self._dt_inv = round(1 / dt)
        self._load_motions(motion_file, motion_order)
    ...
    def sample_time(self, motion_ids):
        ...
        motion_time = (motion_time // self._dt) * self._dt
        return motion_time
    ...
    def calc_motion_frame(self, motion_ids, motion_times):
        ...
        dof_pos = self.joint_rot_to_dof(joint_rot)

        root_pos_offset = self._calc_loop_offset(motion_ids, motion_times)
        root_pos += root_pos_offset

        return root_pos, root_rot, root_vel, root_ang_vel, joint_rot, dof_pos, dof_vel
    ...
    def _extract_frame_data(self, frame: torch.Tensor, motion_order: list):
        root_pos, root_rot, joint_dof = extract_pose_data(frame)
        kin_model_order = self._kin_char_model.get_joint_order()
        motion_idx = [
            motion_order.index(joint_name) for joint_name in kin_model_order[1:]
        ]
        ...
        motion_idx = torch.tensor(motion_idx, dtype=torch.long, device=self._device)
        joint_dof = joint_dof[:, motion_idx]
    ...
    def _load_motions(self, motion_file, motion_order):
        self._load_motion_pkl(motion_file, motion_order)
    ...
    def _load_motion_pkl(self, motion_file, motion_order):
        ...
        for f in range(num_motion_files):
            ...
            root_pos, root_rot, joint_rot = self._extract_frame_data(
                frames, motion_order
            )
        ...
        self._precompute_motion_steps()

    def _precompute_motion_steps(self):
        self._step_root_pos = []
        self._step_root_rot = []
        self._step_root_vel = []
        self._step_root_ang_vel = []
        self._step_joint_rot = []
        self._step_dof_pos = []
        self._step_dof_vel = []
        num_motions = self.get_num_motions()
        print(f"Precomputing motion steps for {num_motions} motions...")
        for m in range(num_motions):
            motion_times = torch.arange(
                0, self._motion_lengths[m], self._dt, device=self._device
            )
            motion_ids = torch.full(motion_times.shape, m, device=self._device)
            root_pos, root_rot, root_vel, root_ang_vel, joint_rot, dof_pos, dof_vel = (
                self.calc_motion_frame(
                    motion_ids=motion_ids,
                    motion_times=motion_times,
                )
            )
            self._step_root_pos.append(root_pos)
            self._step_root_rot.append(root_rot)
            self._step_root_vel.append(root_vel)
            self._step_root_ang_vel.append(root_ang_vel)
            self._step_joint_rot.append(joint_rot)
            self._step_dof_pos.append(dof_pos)
            self._step_dof_vel.append(dof_vel)

        self._step_root_pos = torch.cat(self._step_root_pos, dim=0)
        self._step_root_rot = torch.cat(self._step_root_rot, dim=0)
        self._step_root_vel = torch.cat(self._step_root_vel, dim=0)
        self._step_root_ang_vel = torch.cat(self._step_root_ang_vel, dim=0)
        self._step_joint_rot = torch.cat(self._step_joint_rot, dim=0)
        self._step_dof_pos = torch.cat(self._step_dof_pos, dim=0)
        self._step_dof_vel = torch.cat(self._step_dof_vel, dim=0)

    def get_precomputed_motion_step(self, motion_ids, motion_times):
        motion_frame = (motion_times * self._dt_inv).long()
        motion_frame = torch.clip(motion_frame, 0, self._step_root_pos.shape[-2] - 1)
        frame_start_idx = self._motion_start_idx[motion_ids]
        idx = motion_frame + frame_start_idx

        return (
            self._step_root_pos[idx],
            self._step_root_rot[idx],
            self._step_root_vel[idx],
            self._step_root_ang_vel[idx],
            self._step_dof_pos[idx],
            self._step_dof_vel[idx],
        )
